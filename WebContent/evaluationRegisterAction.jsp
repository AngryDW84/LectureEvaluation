...
<%
	...
	if (request.getParameter("lectureYear") != null) {
		try {
			...
			System.out.println("입력 연도 데이터 오류") ;
		}
	}
	...
	int result = evaluationDAO.write(new EvaluationDTO(0,userID, lectureName, professorName, lectureYear,
			...
			creditScore,comfortableScore,lectureScore,totalScore, 0));
	
		System.out.println(userID+"|"+ lectureName +"|"+ professorName+"|"+ lectureYear+"|"+
				semesterDivide+"|"+ lectureDivide+"|"+evaluationTitle+"|"+ evaluationContent+"|"+
				creditScore+"|"+comfortableScore+"|"+lectureScore+"|"+totalScore ) ;
	...
	if (result == -1) {
		PrintWriter script = response.getWriter();
		script.println("<script>");
		script.println("alert('강의평가 등록실패.') ; ");
		script.println("history.back();");
		script.println("</script>");
		script.close();
		return;
